...
let currentModel, oldCam, saved;

async function initObject() {
    //    engine = new BABYLON.NativeEngine();
    //    scene = new BABYLON.Scene(engine);
    
    let path = typeof globalThis.modelPath !== "undefined" ? globalThis.modelPath : "";
    console.log("PlaygroundActivity BJS, Loading: " + path)
    
    try {
        const scene = await BABYLON.SceneLoader.AppendAsync(path);
        BABYLON.Tools.Log("Loaded");
        currentModel = scene.meshes[0];
        if (typeof window.onSceneLoaded === 'function') {
        ...
        console.error("Failed to load model or render:", err);
    }
...
function loadModel() {
    if (currentModel) {
        console.log("current model not null");
//        oldCam = scene.activeCamera;
//        saved = {
//            pos: oldCam.position.clone(),
//            target: oldCam.getTarget().clone()
//        };
        currentModel.getChildren().forEach(m => m.dispose());
        currentModel.dispose();
        
        loadNewModel()
        .then(() => {
            console.log("loadNewModel Model loaded and render loop started!");
            //        scene.activeCamera = oldCam;
            //        oldCam.getScene().removeCamera(oldCam);
            //        oldCam.position.copyFrom(saved.pos);
            //        oldCam.setTarget(saved.target);
        })
        .catch(err => {
            console.error("loadNewModel Error loading model:", err);
        });
        
    } else {
        initObject()
        .then(() => {
            console.log("initObject Model loaded and render loop started!");
        })
        .catch(err => {
            console.error("initObject Error loading model:", err);
        });
    }
}
